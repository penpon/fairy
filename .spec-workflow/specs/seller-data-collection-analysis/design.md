...
#### 品質基準
- **Test Coverage**: 80%以上（pytest-cov）
...
- タイトル抽出: **日本語トークン化（MeCab/Janome）で先頭2トークンを抽出**
  ...
  - トークナイザーのインポートエラーは clear メッセージ付きでハンドル
...
- **CI/CD**: GitHub Actions（pytest実行、カバレッジ80%以上を必須）
...
4. `pytest --cov=modules --cov-report=html`（80%以上必須）
