...
1. WHEN システムがYahoo Auctions（https://auctions.yahoo.co.jp/）にアクセス THEN システムはBASIC認証付きプロキシサーバー（http://164.70.96.2:3128）経由で接続する
...
- **Session Storage**: セッションCookieはローカルファイルに保存（将来的には暗号化を検討）
